...
foreach ($output as $line) if (strstr($line, "ffmpeg"))
	{
		...
		echo "Transcoder Active (".$columns[1]." CPU: ".$columns[2]." Mem: ".$columns[3].") <a target=_blank href='ls_transcoderoff.php?n=" . $stream . "'>Close</a>" . $line;
		$transcoding = 1;
		//var_dump($columns);
	}
...
//audio transcode only, when using h264 web encoding
	$cmd ="/usr/local/bin/ffmpeg -c:v libx264 -c:a libfdk_aac -b:a 96k -threads 1 -f flv " . $rtmp_server . "/i_". $stream . " -i " . $rtmp_server ."/". $stream . " >&$log_file & ";
